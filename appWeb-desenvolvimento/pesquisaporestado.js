...
// Atualize a rota de pesquisa para aceitar múltiplos estados
app.post('/pesquisaporestado', async (req, res) => {
    const { estado, nomeArquivo, outrasCondicoes, pagina = 1, itensPorPagina = 100 } = req.body;
    try {
        if (!estado || !nomeArquivo) {
            return res.status(400).json({ error: 'Parâmetros inválidos' });
        }

        await fazerPesquisa(estado, nomeArquivo, outrasCondicoes);
...
app.get('/estados.json', (req, res) => {
    const filePath = path.join(publicFolderPath, 'valoresUnicosCID10.json');
    res.sendFile(filePath);
});

app.get('/abreviacoesestados.json', (req, res) => {
    const filePath = path.join(publicFolderPath, 'abreviacoesEstados.json');
    res.sendFile(filePath);
});

app.get('/pesquisaporestado.html', (req, res) => {
    const filePath = path.join(publicFolderPath, 'pesquisaporestado.html');
    res.sendFile(filePath);
});
